...
# 10. CuAu cluster expansion — fixed composition, N = 16

2x2x4 fcc supercell, 16 sites, 8 Cu / 8 Au, T = 500 K, real cluster-expansion
energy (`data/cuau/CI_params_ECI_CuAu_Final_Submission.json`). The state space
is enumerable, `|Omega| = C(16,8) = 12870`, so TV against the exact Boltzmann
law is computed, not estimated. 200,000 samples per evaluation. iid TV floor
0.00492086 (sd in `iid_TV_floor_sd`).

## 10.1 Non-Dirac (Haar-like) source, single-swap generator

| arm | iters | sym aug | final TV | best TV (iter) | empirical TV | energy TV | dE meV/atom | viol |
|---|---:|---|---:|---:|---:|---:|---:|---:|
| nd s0 | 3000 | yes | 0.17070 | 0.16299 (2700) | 0.17168 | 0.17040 | 4.925 | 0 |
| nd s1 | 3000 | yes | 0.15958 | 0.13236 (2900) | 0.16143 | 0.11619 | 3.314 | 0 |
| ndsym s0b | 8000 | yes | 0.11914 | 0.11759 (7600) | 0.12037 | 0.11828 | 3.359 | 0 |
| ndsym s1b | 8000 | yes | 0.14991 | 0.11498 (7600) | 0.15164 | 0.12703 | 3.668 | 0 |

Both 8000-iteration best values land at iteration 7600, so the published
`ndsym` headline is a long-budget number, not a 3000-iteration one.

## 10.2 Radial (shell) controller, non-Dirac source, CE-aware Hungarian pairing

Normalized shell controller, `gamma_total = 10`, 512 steps, hidden 512,
`clamp = 20`, 3000 iterations, seed 1. `S0` is 100 % shell j = 1; `S3-4` is the
4 -> 2 -> 1 schedule on shells {1, 2, 4}. `C` is the number of mixture
components in the shell kernel (C = 1 reproduces the product controller
exactly); `b-clamp` bounds the log-rate head.

| arm | schedule | sym aug | C | b-clamp | final TV | best TV (iter) | energy TV | ev/path | viol |
|---|---|---|---:|---:|---:|---:|---:|---:|---:|
| S3-4 nosym | S3-4 | no | 1 | 10 | 0.83074 | 0.66657 (800) | 0.27243 | 12.42 | 0 |
| S3-4 sym | S3-4 | yes | 1 | 10 | 0.27011 | 0.22098 (2900) | 0.27009 | 14.93 | 0 |
| S3-4 sym C4 | S3-4 | yes | 4 | 10 | 0.24196 | 0.24196 (3000) | 0.24184 | 18.38 | 0 |
| S0 sym | S0 | yes | 1 | 10 | 0.31506 | 0.17555 (2200) | 0.25891 | 19.78 | 0 |
| S0 sym C4 | S0 | yes | 4 | 10 | 0.22553 | 0.18908 (1800) | 0.22523 | 18.25 | 0 |
| S0 sym bc3 | S0 | yes | 1 | 3 | 0.23779 | 0.21217 (2000) | 0.23770 | 20.25 | 0 |

Symmetry augmentation is the only large effect: 0.66657 -> 0.22098 best TV at
matched schedule S3-4. The mixture kernel (C = 4) and the tighter log-rate bound
(b-clamp 3) do not improve on the C = 1, b-clamp 10 arms at matched budget; the
b-clamp 3 arm cuts peak evaluations per path from 479 to 164 without moving TV.
Every radial arm ran 3000 iterations against the 8000-iteration `ndsym`
headline of 0.11498, so the gap in §10.1 vs §10.2 is not budget-matched. A
matched 8000-iteration `S0 sym` arm is in flight
(`json/results_cuau_radialnd_16_500K_S0_g10_sym_8k_s1.json`).

All arms: 0 constraint violations, mass leak <= 1.1e-16.

```bash
# single-swap non-Dirac
python -m iasbs.cuau train-nd --size 2 2 4 --temp 500 --gamma-total 10 \
    --steps 512 --iters 8000 --batch 256 --hidden 512 --sym-aug \
    --n-samples 200000 --seed 1 --tag ndsym16_s1b \
    --out json/results_cuau_ndsym_16_500K_g10_s1b.json

# radial, S0, symmetry augmentation, CE pairing
python -m iasbs.cuau_radial train-nd --size 2 2 4 --temp 500 --schedule S0 \
    --shells 1 --gamma-total 10 --steps 512 --iters 3000 --batch 256 --mb 512 \
    --inner 5 --inner-h 5 --buffer 20 --hidden 512 --components 1 \
    --b-clamp 10 --lr 1e-3 --lr-h 1e-3 --pairing ce --sym-aug \
    --eval-every 100 --n-samples 200000 --seed 1 --ckpt-dir ckpt \
    --tag radnd16_S0sym_s1 \
    --out json/results_cuau_radialnd_16_500K_S0_g10_sym_s1.json

# swap --components 4 for the mixture arms, --b-clamp 3 for the bc3 arm,
# --schedule S3-4 --shells 1 2 4 for the S3-4 arms, drop --sym-aug for nosym.
```
